<template>
  ...
  说明:
    ...
    <el-table :data="configList" border stripe @cell-click="cellClick" style="width:100%">
      ...
      <el-table-column align="center" prop="type" label="数据类型" min-width="120">
        <template slot-scope="scope">
          <el-select
      ...
      <el-table-column prop="value" label="值" min-width="200">
        <template slot-scope="scope">
          <div v-if="scope.row.type !== undefined && scope.row.editValue">
            ...
            ></el-input>
            <el-checkbox v-model="scope.row.value" ref="inputValue" v-if="scope.row.type === 2">{{
              scope.row.value ? '是' : '否'
            }}</el-checkbox>
            <el-input
              v-model.trim="scope.row.value"
      ...
      <el-table-column prop="describe" label="说明" min-width="200">
        <template slot-scope="scope">
          <el-input
      ...
      <el-table-column align="center" label="操作" width="80">
        <template slot-scope="scope">
          <el-button type="text" class="el-icon-delete" @click="deleteConfig(scope.row)">
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  computed: {
    isFull() {
      return this.configList.every(
        item => item.name !== '' && item.type !== '' && item.value !== ''
      )
    },
    isEdited() {
  ...
  methods: {
    ...
    cellClick(row, event) {
      if (event.label !== '操作') {
        ...
        let property = this.$utils.firstUpperCase(event.property)
        this.disabledEdit()
    ...
    disabledEdit() {
      this.configList.forEach(item => {
        ...
        console.log('TCL: disabledEdit -> item', item)
      })
    },
    // 数据类型下拉框变化
    selectChange(row) {
